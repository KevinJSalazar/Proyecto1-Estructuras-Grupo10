...
public class LoginController implements Initializable {
    ...
    private void login(MouseEvent event) {
        ...
        if(usuario.isEmpty()|| contraseña.isEmpty())
            UtileriaMensajes.generarAlertaInfo("Ingreso inválido", "Debe ingresar su usuario y contraseña.");
         // Por medios prácticos, simplemente haremos lo siguiente
        else{
            if(Usuario.verificarCuenta(usuarios, usuario, contraseña)){
                if(UtileriaMensajes.generarAlertaConfirmacion("Acceso permitido", "Permiso concedido. ¡Usuario correcto!")){
                    Usuario usuarioCuenta = Usuario.filtrarUsuario(usuarios, usuario);
                    UtileriaFunciones.cambiarDashboardPrincipal(usuarioCuenta);
                }
            } else{
                UtileriaMensajes.generarAlertaError("Ingreso inválido", "Usuario y/o contraseña incorrectos.");
            }
        }
    }
